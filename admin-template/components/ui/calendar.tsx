...
function Calendar({
  ...
  buttonVariant = "ghost",
  /** How many years after the reference year to include in the year dropdown (react-day-picker defaults to the current calendar year only). */
  yearDropdownYearsAhead = 10,
  formatters,
  components,
  ...props
}: React.ComponentProps<typeof DayPicker> & {
  buttonVariant?: React.ComponentProps<typeof Button>["variant"]
  yearDropdownYearsAhead?: number
}) {
  const defaultClassNames = getDefaultClassNames()

  const referenceDate = props.today ?? new Date()
  ...
  const toYear =
    ...
    props.toMonth === undefined
      ? referenceYear + yearDropdownYearsAhead
      : undefined)
